...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  window.adminAccount = data.admin || { user: 'admin', pass: '' };
  ...
  /* --- 1.3 ADMIN SECURITY --- */
  v('adminUser', window.adminAccount.user);
  // We don't show the password for security
  v('adminPass', '');
    ...
    packsContainer.innerHTML = window.packs.map((pack, i) => {
      ...
      <div class="service-item">
        ...
        <div class="pack-lang-content" id="pack_${i}_ar" style="display:none;" dir="rtl">
          <div class="grid-2">
            <div><label>العنوان والسعر (مثال: موقع احترافي — 999 درهم)</label><input type="text" value="${esc(pack.ar.title)}" onchange="window.packs[${i}].ar.title=this.value; autoSave()"></div>
  ...
  function addPackAction() {
    window.packs.push({
      fr: { title: 'Nouveau Plan — 0 DH', desc: '', features: '' },
      en: { title: 'New Plan — 0 DH', desc: '', features: '' },
      ar: { title: 'خطة جديدة — 0 درهم', desc: '', features: '' }
    });
    renderPacks();
  ...
  document.getElementById('saveAllBtn').addEventListener('click', async () => {
    ...
    const newAdminUser = val('adminUser');
    const newAdminPass = val('adminPass');
    data.admin = { user: newAdminUser || 'admin' };
    if (newAdminPass) {
      data.admin.pass = newAdminPass;
    } else if (window.adminAccount.pass) {
      data.admin.pass = window.adminAccount.pass;
    }
